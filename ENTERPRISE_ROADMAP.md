# CRUCIBLE — Enterprise Integration Roadmap
...
CRUCIBLE must speak the systems those teams already live in. This document
...
|--------|------|-------------|
| **Jira** | Dev-team ticket lifecycle (adapter exists — extend it) | Jira Webhooks: notify CRUCIBLE when issue → "Done" |
...
**Note:** MTTR only means something if CRUCIBLE owns the ticket lifecycle —
...
**Practical path:** Let admin configure `blended_engineer_hourly_rate` in `.crucible.yml`.
...
Requires connecting each CRUCIBLE run to a deployment event.

### Systems Required

| System | Role | Integration |
|--------|------|-------------|
| **Jenkins** | Most common enterprise CI — trigger CRUCIBLE on build | Jenkins Post-Build plugin or pipeline step with `BUILD_NUMBER` tag |
...
| **NIST NVD API** | CWE → CVE enrichment → CVSS → compliance impact | Live pull (partially in CRUCIBLE already) |

**Note:** ServiceNow GRC is the gatekeeper in every regulated enterprise.
If CRUCIBLE findings don't appear as evidence artifacts in ServiceNow GRC,
...
If CRUCIBLE events don't appear in Splunk, the SOC team will never see them.
...
3. **Splunk HEC** — gets CRUCIBLE into SOC workflow with zero behavioral change on their part
