...
- [If your team adopted TDD, you already understand why CRUCIBLE exists](#if-your-team-adopted-tdd-you-already-understand-why-crucible-exists)
...
- [The twelve levers of resilience](#the-twelve-levers-of-resilience-crucible-tests)
...
- [Competitive positioning](#crucible-in-the-security-toolchain--competitive-positioning)
- [How CRUCIBLE works](#how-crucible-works)
...
CRUCIBLE both **exposes** an MCP server (so your IDE can trigger and poll runs) and **consumes** external MCP servers plus two free live-CVE feeds (so every run is enriched with current threat data). These are two independent mechanisms — full detail, including exactly which IDEs are confirmed supported for which purpose, is in [Domain Intelligence → CRUCIBLE as an MCP server vs. consumer](DOMAIN_INTELLIGENCE.md#crucible-as-an-mcp-server-vs-crucible-as-an-mcp-consumer).
...
crucible_policy_list — list available security domain playbooks
...
`crucible_run` fires an `asyncio.Task` and returns the `run_id` immediately, because a full run can take anywhere from under a minute to several minutes depending on mode and model provider — far longer than MCP's interactive-use expectations. `crucible_status` polls until it's done. For team deployments, `crucible serve --mcp` exposes the same tools over HTTP alongside the Combat Dashboard, so a whole team can share one CRUCIBLE instance.
...
CRUCIBLE is a mechanism for the layer network security cannot reach — it does not replace network security, and it is not a substitute for the tools covered in [Competitive Positioning](#crucible-in-the-security-toolchain--competitive-positioning) below.
...
**2. Correctness of defense** — having a defense is not enough if it is bypassable. CRUCIBLE scores partial defenses at 0.5, not 1.0, because a partial defense gives false confidence.
...
CRUCIBLE's Breaker reads the **specification**. It asks: "Given what this code is *supposed to do*, what would a motivated attacker *try to do with it*?" — a different question, and the one that matters for security.
...
3. **The spec survives implementation changes.** Refactor the code and CRUCIBLE's attacks against the spec remain valid — they test intent, not implementation.
...
**ARS is tamper-evident.** Every report carries a SHA-256 hash over the ordered attack array. `crucible verify <run_id>` re-derives this hash at any future point.

**ARS is a gate, not a suggestion.** With `fail_open: false`, ARS below `minimum_ars` causes `crucible run` to exit 1 — blocking the CI pipeline.
...
A scanner that runs **after** code is built attacks the **implementation** — what the AI decided to do. CRUCIBLE's Breaker, running concurrently from the **specification**, attacks the **intent** — what the system was supposed to do. Attackers attack intent, not implementation.

### The anchoring problem that sequential testing cannot escape

A human reviewer (or a post-build scanner) who sees code first unconsciously anchors to "what does this code do?" CRUCIBLE's Breaker **never sees the generated code** — it reasons purely from the specification, without any anchor to the Builder's implementation choices.

### The commitment trap — why post-build security review fails in practice

By the time a security review runs on generated code: the AI generated it, a developer approved the PR, CI passed, and the team has moved on. A vulnerability found now requires a new PR, a re-context-switch, a new review cycle — under timeline pressure, because the feature was "already done." In practice, findings from post-build review get negotiated, deferred, or closed as "acceptable risk" at a rate pre-commit findings simply are not. CRUCIBLE catches findings before the first commit exists — there is no sunk cost to overcome.
...
Traditional security testing operates at code review or QA. CRUCIBLE operates at the design/implementation boundary. The cost difference is not a multiplier — it is an order of magnitude.

### What concurrent execution is NOT

- **Not a replacement for penetration testing.** Human pentesters with full system access will always discover attacks CRUCIBLE misses.
- **Not a SAST/DAST replacement.** CRUCIBLE does not scan your existing codebase; it adversarially tests each spec-to-code generation event.
...
Every time a new AI coding tool runs, it starts cold. After every CRUCIBLE run, every scored attack is written into the **Knowledge Forge** — a [ChromaDB](https://www.trychroma.com/) vector database embedded in `.crucible/forge/`. On the next run, the Breaker recalls the most semantically similar past attacks for this codebase fingerprint and starts from a higher adversarial baseline — a learning curve rather than a cold start every time.
...
Beyond language, CRUCIBLE detects surface signals that select sub-profiles: async/Promise patterns prioritize race conditions; filesystem access prioritizes path traversal; `eval`/dynamic exec prioritizes injection; React/Next.js prioritizes XSS and CORS; NestJS/Spring prioritizes authentication and BOLA; Go web frameworks prioritize SSRF and resource exhaustion.
...
Enabled by default; disable per-project in `.crucible.yml`:
...
crucible policy hub                             # browse community-contributed domains
crucible policy install owasp_api_security      # install one
crucible policy search "broker dealer"          # search by tag or keyword
```

### Feature 7 — SARIF 2.1.0, JUnit XML, and HTML Resilience Reports

CRUCIBLE emits three output formats simultaneously from every run, each for a different audience:
...
```bash
crucible run --issue spec.md --output-sarif crucible.sarif --output-junit crucible.xml
...
```bash
crucible leaderboard --reports-dir .crucible/reports --output docs/leaderboard.html
```
...
Not everyone sharing a CRUCIBLE instance should have the same permissions. Three roles, mapped to GitHub team membership, cached 5 minutes; role lookups degrade gracefully to `DEVELOPER` (least privilege) on API unavailability.
...
| **Developer** | View reports, download evidence (read-only) | `CRUCIBLE_DEV_TEAMS` (default: any authenticated user) |
...
A failed CI gate stops a merge but doesn't, by itself, notify a security team or create a tracking ticket. When ARS falls below `minimum_ars`, CRUCIBLE can dispatch a Slack attachment (top missed attacks, link to the report) and/or a Jira ticket (full attack details, CWE references, run ID). Both are best-effort — a notification failure never blocks the CI gate or report generation.
...
For organizations where sending a specification to an external API is itself a compliance risk (PHI, defense, data sovereignty constraints): CRUCIBLE's entire engine runs offline using [Ollama](https://ollama.ai). No internet connection required. ChromaDB runs embedded — no external vector DB service needed.
...
```bash
docker compose up -d    # ChromaDB + CRUCIBLE + (optionally) Ollama, self-contained
```
...
CRUCIBLE posts the ARS as a commit status. With `GITHUB_TOKEN` set, it also adds a PR comment with the full attack table. `fail_open: false` in `.crucible.yml` blocks the merge if ARS falls below the gate.
...
A fair question. Before evaluating CRUCIBLE, it's worth understanding what each category of existing tool does — and the structural gap none of them close.
...
**CRUCIBLE is not a replacement for SAST, SCA, DAST, or penetration testing.** These tools cover different surfaces with different techniques at different time horizons. CRUCIBLE adds the layer none of them provide: adversarial assessment at the generation event.
...
### `crucible setup` — configure model and integrations
...
### `crucible init` — scaffold config

```bash
crucible init                     # scaffold .crucible.yml with defaults
crucible init --domain hipaa      # scaffold with a specific domain active
crucible init --force             # overwrite existing .crucible.yml
```
...
### `crucible status` — running and recent runs

```bash
crucible status              # recent 10 runs
crucible status --all        # all completed runs
```

### `crucible findings` — vulnerability findings, fix-first

```bash
crucible findings                    # last run
crucible findings <run_id>           # specific run
crucible findings --format md        # markdown output for PRs
```

### `crucible compare` — diff two runs

```bash
crucible compare <run_id_a> <run_id_b> --pretty
```

### `crucible learn` — feed a run into the Knowledge Forge

```bash
crucible learn <run_id> --pretty
```
...
### `crucible audit` — compliance audit

```bash
crucible audit                    # last 90 days, all domains
crucible audit --days 30          # custom window
crucible audit --domain hipaa     # filter by policy domain
```
...
crucible policy validate <path-to-yaml>         # validate a custom domain's schema
```

### `crucible integrate` — one-command IDE/CI wiring

```bash
crucible integrate                        # configure everything
crucible integrate --platform claude-code
crucible integrate --platform github-actions
crucible integrate --dry-run              # preview changes without writing files
```

Platforms: `claude-code`, `cursor`, `windsurf`, `copilot`, `codex`, `github-actions`, `all`.

### `crucible mcp-server` — MCP server, stdio transport

```bash
crucible mcp-server
```

Confirmed local IDE support: Claude Code, Cursor, Windsurf, Zed. See [Domain Intelligence](DOMAIN_INTELLIGENCE.md#crucible-as-an-mcp-server-vs-crucible-as-an-mcp-consumer).
...
- **CPaaS Mode** — `crucible serve` runs as a persistent GitHub App webhook server; every PR is adversarially tested automatically with zero developer workflow change. Requires `GITHUB_APP_ID`, `GITHUB_PRIVATE_KEY_PATH`, `GITHUB_WEBHOOK_SECRET`.
...
CRUCIBLE ships with 554 tests across all major components (`pytest --collect-only -q` to confirm the current count yourself). Test coverage is a first-class commitment.
...
| `test_mcp_server.py` | CRUCIBLE-as-MCP-server tool exposure |
...
This repository ships with seven Claude Code slash commands in `.claude/skills/crucible/` — productivity tools for contributors who use Claude Code. They do not affect CRUCIBLE's runtime behavior.
...
**New policy domains** — more regulatory frameworks. Copy `src/crucible/policy/domains/hipaa.yaml` and follow the schema (see [Domain Intelligence → Bring your own domain](DOMAIN_INTELLIGENCE.md#bring-your-own-domain)).
...
Attack count is fixed by mode (quick=5, standard=20, thorough=50); wall-clock time is not — it depends almost entirely on which model provider you configure, from single-digit seconds with a fast paid API to several minutes with a free-tier or local model. Run `crucible doctor` after setup to see what your specific configuration will look like in practice.

**Q: Is the ARS defensible to a security auditor?**
CRUCIBLE produces NIST SSDF, SOC 2, and ISO 27001 control mapping artifacts with a SHA-256 integrity hash. `crucible verify` re-derives the hash at any future audit, independent of CRUCIBLE itself.
...
Yes. Copy `src/crucible/policy/domains/hipaa.yaml`, follow the schema, add tests, open a PR. See [Domain Intelligence → Bring your own domain](DOMAIN_INTELLIGENCE.md#bring-your-own-domain).
