...
domain, what does CRUCIBLE actually cover today, what is static content versus
...
you point CRUCIBLE at a regulated codebase.
...
- [CRUCIBLE as an MCP server vs. CRUCIBLE as an MCP consumer](#crucible-as-an-mcp-server-vs-crucible-as-an-mcp-consumer)
...
Five domains ship in `src/crucible/policy/domains/` and are available with no
...
`crucible policy list` yourself to confirm):
...
```bash
crucible run --issue spec.md --domain hipaa
crucible run --issue spec.md --domain finra
```

If no `--domain` is passed, CRUCIBLE defaults to `owasp_top10`.
...
| **Custom enterprise MCP server** (e.g. an internal FINRA threat-intel feed) | **Live, if you provide one** | Whatever the server returns | You operate and configure it — CRUCIBLE provides the plumbing, not the feed |
| The 5 built-in policy domains themselves (FINRA, HIPAA, OWASP Top 10, PCI DSS, SOC2) | **Static** — versioned YAML, updated via CRUCIBLE releases | Domain-specific scenarios | Included, no setup |

The honest way to say this: **CRUCIBLE does not ship a dedicated "live FINRA
...
is exactly the extension point for wiring that in. CRUCIBLE does not bundle
one because CRUCIBLE does not have a relationship with FINRA, HIPAA regulators,
...
- Fetched fresh at the start of every CRUCIBLE process (no stale cache across runs)
...
## CRUCIBLE as an MCP server vs. CRUCIBLE as an MCP consumer

These are two independent, unrelated MCP relationships that both happen to
exist in CRUCIBLE. Keeping them separate matters:

|  | **CRUCIBLE as MCP server** | **CRUCIBLE as MCP consumer (DIA)** |
|---|---|---|
| Direction | Your IDE calls CRUCIBLE | CRUCIBLE calls an external server |
| Purpose | Let a coding tool trigger and poll adversarial runs | Enrich a run with live external threat data |
| Launch | `crucible mcp-server` (stdio) or `crucible serve --mcp` (HTTP) | Configured via `mcp_servers:` in `.crucible.yml` |
| Tools involved | 5 tools CRUCIBLE exposes (below) | Whatever tool the external server exposes (you choose the tool name in config) |
...
### CRUCIBLE as an MCP server

Exposes 5 tools over stdio (`crucible mcp-server`) or HTTP (`crucible serve
--mcp`), confirmed supported for local stdio integration with **Claude Code,
Cursor, Windsurf, and Zed**:

| Tool | Purpose |
|---|---|
| `crucible_run` | Start an adversarial assessment; returns a `run_id` in under a second |
| `crucible_status` | Poll a `run_id` for progress or the final result |
| `crucible_vault_stats` | Knowledge Forge / Forge Ledger statistics |
| `crucible_policy_list` | List the policy domains available in this project |
| `crucible_verify` | Re-derive and confirm the SHA-256 integrity hash of a stored report |

`crucible_run` returns immediately and the assessment continues as a
...
Separately, `crucible integrate` can auto-generate MCP config and CI wiring
...
### CRUCIBLE as an MCP consumer (DIA)
...
```bash
crucible policy hub                       # browse everything in the hub index
crucible policy install owasp_api_security
crucible policy search "broker dealer"    # search by name, tag, or regulatory framework
```

Installed domains land in `.crucible/policies/` and take precedence over a
...
`raw.githubusercontent.com/sanjoy1234/crucible/main/policy-hub/index.json` —
i.e. it reads directly from this repository's own default branch. That means
`crucible policy install` only works once this repository is public on
...
Drop it in `.crucible/policies/<name>.yaml` and reference it with `--domain
<name>`. Validate the schema before relying on it:

```bash
crucible policy validate .crucible/policies/my_custom_domain.yaml
```
