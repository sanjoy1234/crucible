---
name: crucible:learn
description: Ingest a completed CRUCIBLE run into the Knowledge Forge and update the Adaptive Brain's attack effectiveness scores. Call after reviewing a completed run to reinforce learning.
---
...
```
/crucible:learn --run-id <run_id> --outcome pass|fail
```

## Execution

```bash
crucible learn --run-id "$RUN_ID" --outcome "$OUTCOME"
```

## What It Does

1. Reads `.crucible/reports/<run_id>.json`
