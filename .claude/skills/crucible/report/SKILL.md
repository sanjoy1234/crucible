...
description: Render a stored CRUCIBLE Resilience Report as Markdown, HTML, or structured JSON. Can attach to a GitHub PR comment.
---
...
```
/crucible:report --run-id <id> [--format md|html|json] [--post-pr]
```

## Execution

```bash
crucible report --run-id "$RUN_ID" --format "$FORMAT"
```
...
For `--format html`: Save to `.crucible/reports/<run_id>.html` and show path.
