...
description: Dry-run CRUCIBLE validation — parses the spec, resolves policy playbooks, checks model availability, and runs the AVF golden fixture gate. No attacks are fired. Exit 1 if any check fails.
---

# /crucible:validate

Validates the CRUCIBLE environment and spec WITHOUT firing any real attacks.
...
```bash
crucible validate --spec "$SPEC_FILE"
```
...
3. **ChromaDB:** Knowledge Forge writable at `.crucible/forge/`
4. **Playbook resolution:** All domains listed in `.crucible.yml` have corresponding YAML files
