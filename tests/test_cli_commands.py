"""Tests for crucible findings, integrate CLI commands, and GitHub URL loading."""
...
from crucible.cli import main, _parse_github_repo_url, _load_github_repo_spec
from crucible.agents.breaker import Attack
from crucible.core.combat_pair import CombatResult
from crucible.output.report import build_report, generate_run_id, save_report
...
class TestFindingsCommand:
    ...
    def test_explicit_run_id_not_found_exits(self):
        runner = CliRunner()
        with runner.isolated_filesystem():
            result = runner.invoke(main, ["findings", "crucible-nonexistent-run-id"])
        assert result.exit_code == 1
...
class TestIntegrateCommand:
    ...
    def test_dry_run_does_not_write_files(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            gha_written = Path(".github/workflows/crucible.yml").exists()
    ...
    def test_dry_run_github_actions_platform(self, tmp_path):
        ...
        assert result.exit_code == 0
        assert "crucible.yml" in result.output or "github-actions" in result.output.lower()
    ...
    def test_live_github_actions_writes_yaml(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            gha_file = Path(".github/workflows/crucible.yml")
            file_exists = gha_file.exists()
            content = gha_file.read_text() if file_exists else ""
        assert result.exit_code == 0
        assert file_exists
        assert "crucible" in content.lower()
    ...
    def test_live_github_actions_content_valid_yaml(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            content = Path(".github/workflows/crucible.yml").read_text()
...
class TestSetupCommand:
    def test_model_only_skips_integration_prompts_and_writes_provider(self, tmp_path):
        """`crucible setup --model` must reconfigure only the model provider —
