...
from crucible.policy.engine import load_domain, validate_domain_yaml
from crucible.output.notifications import (
    send_slack,
    create_jira_ticket,
    notify_low_ars,
    NotificationResult,
...
def test_send_slack_success(monkeypatch):
    ...
    ok, err = send_slack(
        ...
        run_id="crucible-2026-06-28T12-00-00Z-abcd",
        ars=0.62,
        miss_count=3,
        top_misses=[
...
def test_notify_slack_called_on_low_ars(monkeypatch):
    ...
    monkeypatch.setattr("crucible.output.notifications.send_slack", mock_send_slack)
...
def test_notify_jira_called_when_env_set(monkeypatch):
    ...
    monkeypatch.setattr("crucible.output.notifications.create_jira_ticket", mock_create_jira)
