"""Tests for live progress display, --background flag, and crucible status command."""
...
class TestWriteRunStatus:
    def test_creates_status_file(self, tmp_path):
        ...
        _write_run_status("crucible-test-001", runs_dir, status="running", pid=12345)
        status_file = runs_dir / "crucible-test-001.json"
        assert status_file.exists()
        data = json.loads(status_file.read_text())
        assert data["status"] == "running"
        assert data["pid"] == 12345
        assert data["run_id"] == "crucible-test-001"
...
class TestStatusCommand:
    def test_no_runs_shows_helpful_message(self):
        ...
        assert "No runs found" in result.output or "crucible run" in result.output

    def test_completed_run_shown_in_table(self):
        runner = CliRunner()
        with runner.isolated_filesystem():
            reports_dir = Path(".crucible/reports")
            reports_dir.mkdir(parents=True)
            report = {
                "run_id": "crucible-test-abc",
                "ars_score": 0.85,
                "attack_count": 5,
                "miss_count": 1,
            ...
            (reports_dir / "crucible-test-abc.json").write_text(json.dumps(report))
            result = runner.invoke(main, ["status"])
        assert result.exit_code == 0
        assert "crucible-test-abc" in result.output or "0.850" in result.output

    def test_running_status_shown_with_alive_pid(self):
        runner = CliRunner()
        with runner.isolated_filesystem():
            runs_dir = Path(".crucible/runs")
            runs_dir.mkdir(parents=True)
            # Use current process PID so it appears "alive"
            status = {
                "run_id": "crucible-bg-run",
                "status": "running",
                "pid": os.getpid(),
            ...
            (runs_dir / "crucible-bg-run.json").write_text(json.dumps(status))
            result = runner.invoke(main, ["status"])
        assert result.exit_code == 0
    ...
    def test_dead_pid_removed_from_running(self):
        ...
        with runner.isolated_filesystem():
            ...
            sf = runs_dir / "crucible-dead-run.json"
...
class TestBackgroundFlag:
    ...
    def test_background_prints_run_id_and_status_hint(self):
        ...
        assert "crucible status" in result.output
