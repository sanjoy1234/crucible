"""Tests for CRUCIBLE MCP Server — Sprint A."""
...
from crucible.mcp.server import MCPServer, _McpError, _eta, _ATTACK_COUNTS, _TOOLS
...
def _server(engine=None):
    ...
    return MCPServer(config=CrucibleConfig(), engine_fn=engine or _fake_engine)
...
async def test_tools_list_returns_all_five_tools():
    ...
    assert names == {"crucible_run", "crucible_status", "crucible_vault_stats", "crucible_policy_list", "crucible_verify"}
...
@pytest.mark.asyncio
async def test_crucible_run_has_required_spec_field():
    s = _server()
    resp = await s.handle_message({"jsonrpc": "2.0", "id": 6, "method": "tools/list", "params": {}})
    run_tool = next(t for t in resp["result"]["tools"] if t["name"] == "crucible_run")
...
@pytest.mark.asyncio
async def test_crucible_run_returns_run_id():
    s = _server()
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 10,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {"spec": "Build a login endpoint"}},
    })
    result = resp["result"]
    assert result["run_id"].startswith("crucible-mcp-")
    assert result["status"] == "started"


@pytest.mark.asyncio
async def test_crucible_run_empty_spec_returns_error():
    s = _server()
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 11,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {"spec": ""}},
    })
    assert "error" in resp
...
@pytest.mark.asyncio
async def test_crucible_run_missing_spec_returns_error():
    s = _server()
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 12,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {}},
    })
    assert "error" in resp


@pytest.mark.asyncio
async def test_crucible_run_invalid_mode_defaults_to_quick():
    s = _server()
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 13,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {"spec": "spec text", "mode": "ultrafast"}},
    })
...
@pytest.mark.asyncio
async def test_crucible_run_content_includes_run_id():
    s = _server()
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 14,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {"spec": "Build a user profile API"}},
    })
...
@pytest.mark.asyncio
async def test_crucible_status_running_before_completion():
    s = _server()
    # Start a run (engine returns instantly but task hasn't been awaited yet)
    run_resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 20,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {"spec": "Build an API", "mode": "quick"}},
    })
    ...
    status_resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 21,
        "method": "tools/call",
        "params": {"name": "crucible_status", "arguments": {"run_id": run_id}},
    })
...
@pytest.mark.asyncio
async def test_crucible_status_complete_after_await():
    s = _server()
    run_resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 22,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {"spec": "Build a payment API"}},
    })
    run_id = run_resp["result"]["run_id"]
...
@pytest.mark.asyncio
async def test_crucible_status_unknown_run_id_returns_error():
    s = _server()
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 24,
        "method": "tools/call",
        "params": {"name": "crucible_status", "arguments": {"run_id": "nonexistent-run-id"}},
    })
    assert "error" in resp
    assert resp["error"]["code"] == -32602
...
@pytest.mark.asyncio
async def test_crucible_status_engine_error_surfaced():
    s = _server(engine=_error_engine)
    run_resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 25,
        "method": "tools/call",
        "params": {"name": "crucible_run", "arguments": {"spec": "error spec"}},
    })
    run_id = run_resp["result"]["run_id"]
    await asyncio.sleep(0.05)
...
async def test_vault_stats_no_vault_returns_zero():
    ...
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 30,
        "method": "tools/call",
        "params": {"name": "crucible_vault_stats", "arguments": {}},
    })
...
async def test_policy_list_returns_seven_domains():
    ...
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 40,
        "method": "tools/call",
        "params": {"name": "crucible_policy_list", "arguments": {}},
    })
...
async def test_verify_missing_run_id_returns_error():
    ...
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 50,
        "method": "tools/call",
        "params": {"name": "crucible_verify", "arguments": {}},
    })
    assert "error" in resp
    assert resp["error"]["code"] == -32602
...
async def test_verify_nonexistent_report_returns_not_found():
    ...
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 51,
        "method": "tools/call",
        "params": {"name": "crucible_verify", "arguments": {"run_id": "no-such-run"}},
    })
...
async def test_unknown_tool_returns_error():
    ...
    resp = await s.handle_message({
        "jsonrpc": "2.0", "id": 60,
        "method": "tools/call",
        "params": {"name": "crucible_nonexistent", "arguments": {}},
    })
    assert "error" in resp
    assert resp["error"]["code"] == -32601
