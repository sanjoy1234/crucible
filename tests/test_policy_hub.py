"""Tests for Policy Hub — crucible policy install (Sprint 5)."""
...
from crucible.policy.hub import (
    HubDomainEntry,
    InstallResult,
    fetch_index,
    install_domain,
...
from crucible.policy.engine import list_available_domains, load_domain
...
def test_install_domain_already_installed_with_force(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("crucible.policy.hub.fetch_index", mock_fetch)
    import httpx
...
def test_install_domain_not_in_index(tmp_path, monkeypatch):
    monkeypatch.setattr("crucible.policy.hub.fetch_index", lambda _url: [])
...
def test_install_domain_network_error(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("crucible.policy.hub.fetch_index", bad_fetch)
...
def test_search_index_by_name(monkeypatch):
    ...
    monkeypatch.setattr("crucible.policy.hub.fetch_index", lambda _url: entries)
...
def test_installed_domain_appears_in_list_available(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("crucible.policy.engine._USER_POLICIES_DIR", tmp_path)
...
def test_hub_domain_yaml_loadable_via_engine(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("crucible.policy.engine._USER_POLICIES_DIR", hub_domains_dir)
    domain = load_domain("owasp_api_security")
    assert domain.name == "owasp_api_security"
