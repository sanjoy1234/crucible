...
from crucible.service.rbac import (
    Role,
    RbacConfig,
    RbacEnforcer,
    ROLE_LABELS,
...
from crucible.service.config import ServiceConfig
...
def test_rbac_config_from_env(monkeypatch):
    ...
    monkeypatch.setenv("CRUCIBLE_ADMIN_TEAMS", "security-leads,platform-admin")
    monkeypatch.setenv("CRUCIBLE_REVIEWER_TEAMS", "backend-leads")
    monkeypatch.setenv("CRUCIBLE_ALLOW_ANY_AUTH", "false")
...
def test_service_config_rbac_from_env(monkeypatch):
    monkeypatch.setenv("CRUCIBLE_RBAC_ENABLED", "true")
