"""Tests for the CrucibleHarness hook chain."""

from __future__ import annotations

import pytest

from crucible.harness.runner import CrucibleHarness, RunContext
...
def test_register_sync_hook():
    harness = CrucibleHarness.__new__(CrucibleHarness)
...
def test_harness_init_registers_builtin_hooks():
    cfg = CrucibleConfig.load()
    harness = CrucibleHarness(config=cfg)
...
def test_harness_commands_importable():
    from crucible.harness.commands import run, validate, learn, compare
    from crucible.harness.commands import doctor, init, report, verify
