...
> — CRUCIBLE Design Principle 6 (Stanford TKI — Rule 6: Ship Confidence)
...
CRUCIBLE v0.1.0 introduces the world's first production Python library that runs
...
They all generate code and test it after the fact. CRUCIBLE attacks during
...
- **CrucibleHarness** — custom hook chain (pre_run, post_round, post_run, learn); no LangChain, no AutoGen
- **Harness Commands** — programmatic API layer (run, validate, learn, compare, doctor, init, report, verify)
- **Developer CLI Skills** — `/crucible:run`, `/crucible:validate`, `/crucible:learn`, `/crucible:compare`, `/crucible:doctor`, `/crucible:report`, `/crucible:verify`

### CLI

```
crucible run       --issue <spec> --mode quick|standard|thorough
crucible validate  [--spec <file>]
crucible doctor    [--network-check]
crucible learn     <run_id>
crucible compare   <run_id_a> <run_id_b>
crucible audit     [--days N]
crucible policy    list | validate <domain>
crucible stats     [--days N] [--learning-curve]
crucible report    <run_id> [--format md|json]
crucible verify    <run_id>
crucible prune     [--older-than 90d] [--dry-run]
crucible init      [--domain <domain>]
```

### Infrastructure

- **GitHub Action** — posts ARS as PR comment; blocks merge when ARS < threshold
- **Docker** — `docker compose up` for full stack (CRUCIBLE + ChromaDB)
...
By default, CRUCIBLE **blocks** the merge when ARS falls below `gate.minimum_ars` (default: 0.80).
This is intentional. Security is not optional. Set `fail_open: true` in `.crucible.yml` to downgrade
...
2. Environment health (`crucible doctor --network-check`)
3. AVF gate (`crucible validate` — Breaker finds ≥75% of golden CVE fixtures)
4. Unit tests (`pytest tests/ -q` — 50+ tests passing)
5. Standalone demo (`python examples/standalone_demo.py --mode quick`)
6. CLI integration (`crucible init → validate → run`)
7. Report integrity (`crucible verify <latest_run_id>`)
...
```bash
pip install crucible-ai==0.1.0
crucible init  # creates .crucible.yml with defaults
crucible validate  # verify environment
```
...
- `crucible serve` — start CPaaS GitHub App server
...
*CRUCIBLE is MIT licensed. Contributions welcome — see [CONTRIBUTING-PLAYBOOKS.md](CONTRIBUTING-PLAYBOOKS.md).*
