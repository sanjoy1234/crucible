"""CRUCIBLE GitHub App webhook handler — CPaaS mode."""
...
async def handle_webhook(body: bytes, headers: dict, config: ServiceConfig) -> dict:
    ...
    3. Run CRUCIBLE in the configured mode
    ...
    from crucible.harness.commands.run import execute as run_execute

    try:
        result = await run_execute(
            spec=spec,
            mode=config.crucible_mode,
            config_path=config.crucible_config_path,
        )
...
async def _post_pr_comment(
    ...
    from crucible.output.forge_bot import render_pr_comment
...
def create_server(config: ServiceConfig | None = None):
    ...
    Usage:
        uvicorn crucible.service.github_app:app --port 8080
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run: pip install crucible-ai[serve]"
        )


# ASGI app entry point for uvicorn
app = None  # populated lazily on first import with CRUCIBLE_SERVE=1
if os.environ.get("CRUCIBLE_SERVE"):
    app = create_server()
