"""
Enterprise RBAC — GitHub team-based role enforcement for CRUCIBLE CPaaS.
...
class RbacConfig:
    ...
    admin_teams: list[str] = field(default_factory=lambda: _split_env("CRUCIBLE_ADMIN_TEAMS"))
    reviewer_teams: list[str] = field(default_factory=lambda: _split_env("CRUCIBLE_REVIEWER_TEAMS"))
    dev_teams: list[str] = field(default_factory=lambda: _split_env("CRUCIBLE_DEV_TEAMS"))
    allow_any_authenticated: bool = field(
        default_factory=lambda: os.environ.get("CRUCIBLE_ALLOW_ANY_AUTH", "true").lower() == "true"
    )
