...
@dataclass
class ServiceConfig:
    """Configuration for the CRUCIBLE GitHub App service."""
    ...
    port: int = field(default_factory=lambda: int(os.environ.get("CRUCIBLE_SERVICE_PORT", "8080")))
    host: str = field(default_factory=lambda: os.environ.get("CRUCIBLE_SERVICE_HOST", "0.0.0.0"))

    crucible_mode: str = field(default_factory=lambda: os.environ.get("CRUCIBLE_MODE", "standard"))
    minimum_ars: float = field(
        default_factory=lambda: float(os.environ.get("CRUCIBLE_MIN_ARS", "0.80"))
    )
    post_status: bool = field(
        default_factory=lambda: os.environ.get("CRUCIBLE_POST_STATUS", "true").lower() == "true"
    )
    post_pr_comment: bool = field(
        default_factory=lambda: os.environ.get("CRUCIBLE_POST_COMMENT", "true").lower() == "true"
    )
