"""
ARS Leaderboard — processes SWE-bench agent patch outputs through CRUCIBLE Breaker
...
Usage:
  from crucible.leaderboard.engine import LeaderboardEntry, build_leaderboard, render_leaderboard_html
"""
...
def load_agent_scores_from_reports(reports_dir: Path, gate: float = 0.80) -> list[AgentScore]:
    """
    Load ARS scores from existing CRUCIBLE report JSON files.
...
def render_leaderboard_html(
    entries: list[LeaderboardEntry],
    gate_threshold: float = 0.80,
    title: str = "CRUCIBLE ARS Leaderboard",
    generated_at: str = "",
) -> str:
    ...
    empty_msg = "" if entries else '<p class="empty">No leaderboard data yet. Run <code>crucible leaderboard --jsonl scores.jsonl</code> to generate.</p>'
