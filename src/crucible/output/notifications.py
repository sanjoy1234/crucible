...
def send_slack(
    ...
    payload = {
        "attachments": [
            {
                "color": color,
                "fallback": f"CRUCIBLE ARS {ars:.2f} — {verdict}",
                "title": f"🛡️ CRUCIBLE Adversarial Gate — {verdict}",
                "title_link": "",
                "fields": [
                ...
                "footer": "CRUCIBLE Adversarial Co-Generation Engine",
            }
        ]
    }
...
def create_jira_ticket(
    ...
    summary = f"[CRUCIBLE] ARS {ars:.2f} gate failure — run {run_id[:16]}"
    description = (
        f"CRUCIBLE adversarial gate blocked a run with ARS {ars:.2f}.\n\n"
        f"Run ID: {run_id}\n"
        f"Missed attacks: {miss_count}\n\n"
        f"*Unmitigated vulnerabilities:*\n{miss_items}\n\n"
        f"Fix these security issues and re-run CRUCIBLE before merging."
    )
    ...
    payload = {
        "fields": {
            ...
            "labels": ["security", "crucible", "adversarial-gate"],
        }
    }
