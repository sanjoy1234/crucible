"""
CRUCIBLE MCP Server — expose CRUCIBLE as a Model Context Protocol (MCP) server.

Supports two transports:
  stdio  — local IDE integration (Claude Code, Cursor, Windsurf, Zed)
           Launch: crucible mcp-server
  ...
  http   — team/enterprise deployment, served alongside the Combat Dashboard
           Launch: crucible serve --mcp  (POST /mcp)

Protocol: JSON-RPC 2.0 / MCP 2024-11-05

Tools exposed:
  crucible_run         — start adversarial assessment; returns run_id immediately
...
Async run dispatch:
  crucible_run fires an asyncio.Task and returns in <1s.
  The event loop continues processing new messages (crucible_status polls)
  while the engine runs in the background.

Environment variables (never committed — set in shell or .env):
  CRUCIBLE_MCP_PORT — HTTP port for --mcp mode (default: 8080)
"""
...
_TOOLS = [
    {
        "name": "crucible_run",
        "description": (
            "Start a CRUCIBLE adversarial resilience assessment on a specification. "
            "Returns run_id immediately — the engine runs as a background task. "
            "Poll with crucible_status to retrieve results. "
            ...
            "properties": {
                ...
                "domain": {
                    ...
                    "description": (
                        ...
                        "Use crucible_policy_list to see all options."
                    ),
                },
                "language": {
                    "type": "string",
    ...
    },
    {
        ...
        "description": (
            "Poll for the results of a running CRUCIBLE adversarial assessment. "
        ...
        "inputSchema": {
            ...
            "properties": {
                "run_id": {
                    "type": "string",
                    "description": "The run_id returned by crucible_run.",
                },
            },
            "required": ["run_id"],
        },
    ...
    {
        "name": "crucible_policy_list",
        "description": (
            "List all available security domain playbooks that can be used as the 'domain' "
            "parameter in crucible_run. Each domain steers the Breaker toward domain-specific "
...
EngineFunction = Callable[[str, str, str, str, "str | None", CrucibleConfig], Awaitable[dict]]
...
class MCPServer:
    """
    JSON-RPC 2.0 / MCP 1.0 server for CRUCIBLE.

    Maintains a dict of in-flight asyncio.Tasks (one per crucible_run call) and
    their results. Background tasks run concurrently while the event loop processes
    new crucible_status poll requests.
    ...
    def __init__(
        ...
        self._config = config or CrucibleConfig.load()
        self._tasks: dict[str, asyncio.Task] = {}
        self._results: dict[str, dict] = {}
        self._started: dict[str, float] = {}
        self._engine_fn: EngineFunction = engine_fn or _run_crucible_engine
    ...
    async def _handle_tool_call(self, params: dict) -> dict:
        ...
        if name == "crucible_vault_stats":
            return self._tool_vault_stats(args)
        if name == "crucible_policy_list":
            return self._tool_policy_list(args)
        if name == "crucible_verify":
    ...
    async def _tool_run(self, args: dict) -> dict:
        ...
        run_id = f"crucible-mcp-{uuid.uuid4().hex[:8]}"
        ...
        return {
            "content": [{
                ...
                "text": (
                    ...
                    f"domain: {domain}\n\n"
                    f"Poll for results:\n"
                    f"  crucible_status(run_id='{run_id}')\n\n"
    ...
    def _tool_vault_stats(self, args: dict) -> dict:
        vault_dir = Path(".crucible/vault")
        if not vault_dir.exists():
            return {
                "content": [{"type": "text", "text": "No Forge Ledger entries yet. Run a CRUCIBLE assessment to populate the vault."}],
    ...
    def _tool_policy_list(self, args: dict) -> dict:
        ...
        lines = ["Available CRUCIBLE security domain playbooks:", ""]
        for name, n, desc in domains:
            lines.append(f"  {name:<25} {n:>2} scenarios  {desc}")
        lines += ["", "Usage: crucible_run(spec='...', domain='hipaa')"]
...
async def _run_crucible_engine(
    run_id: str,
    spec: str,
    mode: str,
    domain: str,
    ...
    """
    Run the CRUCIBLE CombatPair engine on an inline spec and return a result dict.

    This is the MCP-facing entry point — equivalent to `crucible run` but:
    ...
    cfg.combat_pair.attack_count = _ATTACK_COUNTS.get(mode, 5)
