...
Privacy guarantees:
  - Opt-in only: CRUCIBLE_FORGE_NETWORK_ENABLED=true required
...
All calls are best-effort and never block a `crucible run`.
"""
...
_DEFAULT_HUB_URL = "https://forge-network.crucible.dev"  # future public hub
...
class ForgeNetworkConfig:
    ...
    enabled: bool = field(
        default_factory=lambda: os.environ.get("CRUCIBLE_FORGE_NETWORK_ENABLED", "false").lower() == "true"
    )
    hub_url: str = field(
        default_factory=lambda: os.environ.get("CRUCIBLE_FORGE_HUB_URL", _DEFAULT_HUB_URL)
    )
    min_ars_to_share: float = field(
        default_factory=lambda: float(os.environ.get("CRUCIBLE_FORGE_MIN_ARS", "0.85"))
    )
    contributor_id: str = ""
